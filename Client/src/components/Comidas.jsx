...
const Bebidas = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://dallas-backend-k4rb/foods");
  ...
  return (
    <section>
      <div>
        <h1 className="text-center uppercase text-3xl text-white font-primary font-bold">Comidas</h1>
        <br />
        <div className="">
          {itemsConBebidas.map((item) => {
            ...
            return (
              <div key={id}>
                <h1 className="uppercase text-2xl py-3 text-white font-secundary font-semibold">{tipo}</h1>
                <hr/>
